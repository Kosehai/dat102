...
public class BS_Tre<T extends Comparable<? super T>> implements SoektreInterface<T> {
	...
	public T leggTil(T nyElement) {
		T elm = finn(nyElement);
        if(elm != null)
            return elm;
        leggTil(rot, nyElement);
        return null;
	}
    ...
    //Rekrusivt går igjennom alle blad til tret og velger alltid
    //Lengste "veien"
    private int hoydeRek(BinaerTreNode<T> t){
        if(t == null)
            return -1;
        
        int vHoyd = hoydeRek(t.getVenstre());
        int hHoyd = hoydeRek(t.getHogre());
        if(vHoyd > hHoyd)
            return vHoyd + 1;
        else 
            return hHoyd + 1;
